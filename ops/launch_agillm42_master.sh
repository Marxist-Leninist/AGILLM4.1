...
exec python -u agillm41.py train --preset agillm4_floor --tie_kv --resume_delta /workspace/agillm4_4090_ckpts/agillm42_tiekv_seed.delta.pt \
  --dblock --dblock_blocks "${DBLOCK_BLOCKS:-14}" --dblock_schedule loss_balanced --dblock_warmup_steps 16 \
  --dblock_sigma_curriculum_steps 2000 --dblock_log_every 25 \
  --dblock_sigma_sampling lognormal --dblock_sigma_stratified --dblock_objective_mode stochastic \
  --dblock_ar_prob 0.60 --dblock_sat_prob 0.25 --dblock_nat_prob 0.15 \
  --dblock_ar_loss_tokens 512 --dblock_sat_loss_tokens 512 --dblock_nat_loss_tokens 512 \
  ...
  --grad_checkpoint --dblock_checkpoint_stride 1 --optimizer paged_adamw8bit --sat_every 1 --nat_every 4 \
